...
# ft_irc - Liste des tâches basée sur la trame d'évaluation

## Partie obligatoire

### Vérifications de base
- [ ] Le projet contient un Makefile et compile correctement avec les options requises.
- [ ] Le projet est écrit en C++98.
- [ ] L'exécutable est appelé comme prévu.
- [ ] Une seule instance de `select()` (ou équivalent) est présente dans le code.
- [ ] `select()` est appelé avant chaque acceptation, lecture/réception, écriture/envoi.
- [ ] `errno` n'est pas utilisé pour déclencher une action spécifique après un appel système.
- [ ] Chaque appel à `fcntl()` est effectué comme suit : `fcntl(fd, F_SETFL, O_NONBLOCK);`.
- [ ] Aucune autre utilisation de `fcntl()` n'est présente.

⚠️ Si l'un des points ci-dessus est incorrect, l'évaluation se termine avec une note de **0**.

---

### Offres spéciales de réseautage
- [ ] Le serveur répond correctement aux commandes partielles envoyées avec `nc`.
- [ ] Les autres connexions restent opérationnelles lorsque des commandes partielles sont envoyées.
- [ ] Le serveur reste opérationnel lorsqu'un client est tué de manière inattendue.
- [ ] Le serveur reste stable lorsque `nc` est fermé après l'envoi d'une commande partielle.
- [ ] Lorsqu'un client est suspendu (`^-Z`), le serveur continue de fonctionner normalement.
- [ ] Les inondations sur un canal n'affectent pas le serveur.
- [ ] Les commandes d'un client suspendu sont correctement traitées lorsqu'il redevient actif.
- [ ] Aucune fuite de mémoire n'est détectée pendant ces tests.

---

### Commandes client de base
- [ ] Les commandes d'authentification (PASS, NICK, USER) fonctionnent correctement avec `nc` et le client IRC de référence.
- [ ] Un utilisateur peut rejoindre un canal sans problème.
- [ ] Les messages privés (PRIVMSG) fonctionnent correctement avec différents paramètres.

---

### Opérateur de canal de commandes client
- [ ] Un utilisateur sans privilèges ne peut pas exécuter de commandes d'opérateur de canal.
- [ ] Toutes les commandes d'opérateur de canal (MODE, KICK, TOPIC, INVITE) fonctionnent correctement.
...
- [ ] Le transfert de fichiers fonctionne correctement avec le client IRC de référence.

### Un petit robot
- [ ] Le projet inclut un bot IRC qui fonctionne correctement.
